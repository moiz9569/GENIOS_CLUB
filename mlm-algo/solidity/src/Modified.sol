...
contract GeniosClub {
    ...
    event Reinvest(
        address indexed user, address indexed currentReferrer, address indexed caller, uint8 matrix, uint8 level
    );
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        User memory user = User({id: lastUserId, referrer: referrerAddress, partnersCount: uint256(0)});
    ...
    function updateX12Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        X12 storage referrerMatrix = users[referrerAddress].x12Matrix[level];

        if (referrerMatrix.firstLevelReferrals.length < 3) {
            referrerMatrix.firstLevelReferrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(referrerMatrix.firstLevelReferrals.length));
            ...
            address ref = referrerMatrix.currentReferrer;
            users[ref].x12Matrix[level].secondLevelReferrals.push(userAddress);

            uint8 len = uint8(referrerMatrix.firstLevelReferrals.length);

            if (len == 3 && referrerMatrix.firstLevelReferrals[2] == referrerAddress) {
                emit NewUserPlace(userAddress, ref, 2, level, 9 + len);
            } else if (len == 2 && referrerMatrix.firstLevelReferrals[1] == referrerAddress) {
                emit NewUserPlace(userAddress, ref, 2, level, 6 + len);
            } else if (len == 1 && referrerMatrix.firstLevelReferrals[0] == referrerAddress) {
                emit NewUserPlace(userAddress, ref, 2, level, 3 + len);
            }

            updateX12ReferrerSecondLevel(userAddress, ref, level);
            return;
        }

        referrerMatrix.secondLevelReferrals.push(userAddress);

        X12 storage firstReferrerMatrix = users[referrerMatrix.firstLevelReferrals[0]].x12Matrix[level];
        X12 storage secondReferrerMatrix = users[referrerMatrix.firstLevelReferrals[1]].x12Matrix[level];
        X12 storage thirdReferrerMatrix = users[referrerMatrix.firstLevelReferrals[2]].x12Matrix[level];

        uint8 len = uint8(referrerMatrix.firstLevelReferrals.length);

        if (
            firstReferrerMatrix.firstLevelReferrals.length <= secondReferrerMatrix.firstLevelReferrals.length
                && secondReferrerMatrix.firstLevelReferrals.length <= thirdReferrerMatrix.firstLevelReferrals.length
        ) {
            updateX12(userAddress, referrerAddress, level, 0);
        } else if (secondReferrerMatrix.firstLevelReferrals.length <= thirdReferrerMatrix.firstLevelReferrals.length) {
    ...
    function updateX12(address userAddress, address referrerAddress, uint8 level, int256 x2) private {
        if (x2 == 0) {
            users[users[referrerAddress].x12Matrix[level].firstLevelReferrals[0]].x12Matrix[level]
                .firstLevelReferrals
                .push(userAddress);
            emit NewUserPlace(
                userAddress,
            ...
            //set current level
            users[userAddress].x12Matrix[level].currentReferrer =
                users[referrerAddress].x12Matrix[level].firstLevelReferrals[0];
        } else if (x2 == 1) {
            users[users[referrerAddress].x12Matrix[level].firstLevelReferrals[1]].x12Matrix[level]
                .firstLevelReferrals
                .push(userAddress);
            emit NewUserPlace(
                userAddress,
            ...
            users[users[referrerAddress].x12Matrix[level].firstLevelReferrals[2]].x12Matrix[level]
                .firstLevelReferrals
                .push(userAddress);
            emit NewUserPlace(
                userAddress,
                users[referrerAddress].x12Matrix[level].firstLevelReferrals[2],
                2,
                level,
                uint8(
                    users[users[referrerAddress].x12Matrix[level].firstLevelReferrals[2]].x12Matrix[level]
    ...
    function usersX12Matrix(address userAddress, uint8 level)
        public
        view
        returns (address, address[] memory, address[] memory, bool)
    {
        return (
            users[userAddress].x12Matrix[level].currentReferrer,
