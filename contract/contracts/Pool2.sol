...
contract Pool2 is IPool {
  ...
  constructor(address tokenAddr) {
    ...
    NextWeekStart = block.timestamp + 180 seconds; // Calculate the timestamp for the start of the next week
  ...
  function _updateNewWeek(uint8 level) internal {
    ...
    uint256 weeksPassed = (block.timestamp - LastWeekStart) / 180 seconds; // Calculate the number of weeks passed since the last week start
    CurrentWeek += weeksPassed; // Increment the current week counter by the number of weeks passed
    LastWeekStart += weeksPassed * 180 seconds; // Update the timestamp of the start of the last week
    NextWeekStart += weeksPassed * 180 seconds; // Update the timestamp of the start of the next week
  ...
  function withdrawLostToken(address token) public {
    require(Id0 == msg.sender, "GC: !Id0");

    IERC20 _token = IERC20(token);
    _token.transfer(Id0, _token.balanceOf(address(this)));
  }
}
