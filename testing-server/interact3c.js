...
const GeniosClub = {
  address: "0xaEf09Cc67894cF0d5c1f59a102BD7239dd63b861",
  abi: [
    {
      type: "constructor",
      name: "",
...
const maxDepth = Math.log(3000) / Math.log(4);
console.log("Maximum Depth:", maxDepth);

async function registerUsers(refAddress, depth) {
  if (depth === 0) return;

  const wallets = await fs.readJson("wallets.json");
  const refWallet = ThirdwebSDK.fromPrivateKey(myKey, "fantom-testnet", {
    secretKey:
  ...
  try {
    for (let i = 0; i < 4; i++) {
      ...
      console.log(
        `Depth: ${depth}, User: ${userAddress}, Referrer: ${refAddress}`
      );
      lastId++;
    }

    // Recursively call the function to register 4 users for each of the newly registered users.
    for (let i = 0; i < 4; i++) {
      const newUserAddress = wallets[lastId - 4 + i].address;
      await registerUsers(newUserAddress, depth - 1);
    }
  } catch (error) {
...
// Start the registration process with the initial refAddress and the desired depth.
registerUsers("0xa4672C43B557A54aF2Be66C651f4E74d02e5a2be", 5);
