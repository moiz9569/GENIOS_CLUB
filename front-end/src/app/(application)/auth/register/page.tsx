...
import { formatUSDT, parseUSDT } from '@/lib/utils/token';
...
const Page = () => {
  ...
  const { contract: USDTContract } = useContract(USDTAddress, USDTAbi);

  console.log(address)
  console.log(USDTAddress)
  console.log(GeniosClubAddress)
  ...
  const callApprove = async () => {
    try {
      const data = await approve({
        args: [GeniosClubAddress, parseUSDT('5')],
      });
  ...
  const balanceFormatted = balance ? ethers.utils.formatUnits(balance, 6) : "0";
  const allowanceFormatted = allowance ? ethers.utils.formatUnits(allowance, 6) : "0";

  const hasBalance = Number(balanceFormatted) >= 5;
  const hasAllowance = Number(allowanceFormatted) >= 5;
